...
void SecureData::code(char key) {
	...
	auto simple = bind(converter, text, key, nbytes, Cryptor());
	thread t1(simple);
	thread t2(simple);
	thread t3(simple);
	thread t4(simple);
...
void SecureData::backup(const char *file) {
	if (!text)
		...
		ofstream actualFile;
		actualFile.open(file, ios::out | ios::binary);
		// TODO: write data into the binary file
		//         and close the file
		actualFile.write(text, nbytes);
		actualFile.close();
	}
}
...
void SecureData::restore(const char *file, char key) {
	...
	ifstream actualFile(file, ios::in | ios::binary);
